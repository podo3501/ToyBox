...
MainSourceExtractor::~MainSourceExtractor() = default;
MainSourceExtractor::MainSourceExtractor(IRenderer* renderer, const wstring& filename, CommandList* cmdList) noexcept :
    m_renderer{ renderer },
    m_filename{ filename },
    m_cmdList{ cmdList },
    m_window{ nullptr }
...
Rectangle MainSourceExtractor::FindRectangleFromMousePosition() const noexcept
{
...
void MainSourceExtractor::Update()
{
...
void MainSourceExtractor::Render() const
{
    RenderProcess();
}
...
//    MainSourceExtractor(renderer, filename, cmdList),
