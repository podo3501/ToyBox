...
bool UIModul2::operator==(const UIModul2& other) const noexcept
{
...
bool UIModul2::SetupMainComponent(const wstring& filename, IRenderer* renderer, const wstring& srcBinderFilename)
{
	m_resBinder = CreateTextureResourceBinder(srcBinderFilename, renderer);
	m_renderer = renderer;
...
void UIModul2::AddRenderer() noexcept
{
	m_renderer->AddRenderComponent(m_mainPanel.get());
}

bool UIModul2::BindTextureResources() noexcept
{
	ReturnIfFalse(m_resBinder);
	ReturnIfFalse(m_mainPanel->BindTextureSourceInfo(m_resBinder.get(), m_renderer->GetTextureController()));

	return true;
}

bool UIModul2::Update(const DX::StepTimer& timer) noexcept
{
	UpdateMouseState();
	return m_mainPanel->ProcessUpdate(timer);
}

void UIModul2::Render(ITextureRender* render) const
{
	m_mainPanel->ProcessRender(render);
}

void UIModul2::UpdateMouseState() noexcept
{
	//hover는 공통으로 호출
...
void UIModul2::UpdateHoverState(vector<IMouseEventReceiver*> receivers, const XMINT2& pos) noexcept
{
...
void UIModul2::ProcessCaptureComponent(const MouseState& mouseState) noexcept
{
	if (!m_capture) return;
...
void UIModul2::CaptureComponent(const MouseState& mouseState) noexcept
{
	if (m_capture) return;
...
void UIModul2::ProcessMouseWheel(int wheelValue) noexcept
{
...
void UIModul2::ReloadDatas() noexcept
{
	m_mainPanel->SetUIModul2(this);
}

void UIModul2::ProcessIO(SerializerIO& serializer)
{
...
bool UIModul2::Write(const wstring& filename) noexcept
{
...
bool UIModul2::Read(const wstring& filename) noexcept
{
	const wstring& curFilename = !filename.empty() ? filename : m_filename;
	SerializerIO::ReadJsonFromFile(curFilename, *this);
	m_mainPanel->PropagateRoot(); //모든 컴포넌트들에 root를 지정.
	m_filename = curFilename;

	return true;
}

bool UIModul2::EnableToolMode(bool enable) noexcept
{
	return m_mainPanel->EnableToolMode(enable);
}

Panel* UIModul2::GetMainPanel() const noexcept { return m_mainPanel.get(); }