...
namespace ComponentTest
{
	...
	static bool AttachComponentHelper(UIComponent* panel, const string& componentName) noexcept
	{
		...
		auto imgGrid1 = CreateImageGrid<ImageGrid1>("ImgGrid1", layout, grid1Source);
		UIComponent* component = panel->GetComponent(componentName);
		return component ? component->AttachComponent(std::move(imgGrid1), { 10, 10 }) : false;
	}
	...
	TEST_F(IntegrationTest, AttachDetachTest)
	{
		...
		EXPECT_EQ(AttachComponentHelper(m_panel.get(), "ImageGrid9"), false);	//9방향 이미지에는 attach 불가
		EXPECT_EQ(AttachComponentHelper(m_panel.get(), "ImageGrid9_1_1"), true);

		EXPECT_EQ(DetachComponentHelper(m_panel.get(), "ImageGrid9_1_1"), false);
		EXPECT_EQ(DetachComponentHelper(m_panel.get(), "ImgGrid1"), true); //위에서 ImgGrid1를 attach 했다.
	}
	...
	TEST_F(IntegrationTest, GetComponents)
	{
		...
		m_panel->AttachComponent(move(img9_2), { 88, 66 });
