...
{
	//TEST_F(ComplexComponent, ListArea) //옮기는거 완료
	//{
	...
	//}
	//
	//TEST_F(ComplexComponent, ListAreaToolMode) //옮길 필요 있을까?
	//{
	//	auto [listArea, listAreaPtr] = GetPtrs(CreateSampleListArea({}));
	//	UIEx(m_main).AttachComponent(move(listArea), { 400, 300 });
	...
	//TEST_F(ComplexComponent, RenderTexture) //옮기는거 완료
	//{
	...
	//TEST_F(ComplexComponent, ScrollBar) //옮기는거 완료
	//{
	...
	//TEST_F(ComplexComponent, Switcher_Scroll) //옮기는거 완료
	//{
	...
	//	testing::MockFunction<void(const XMINT2& pos, InputState)> mockOnPress;
	//	switcherPtr->AddPressCB(mockOnPress.AsStdFunction());

	//	EXPECT_CALL(mockOnPress, Call(testing::_, InputState::Pressed)).Times(1); //Pressed 인자를 넣어서 한번 호출할 것을 기대
	//	EXPECT_CALL(mockOnPress, Call(testing::_, InputState::Held)).Times(1);
