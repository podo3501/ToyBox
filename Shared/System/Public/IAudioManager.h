...
struct IAudioManager
{
	...
	virtual bool LoadSound(const std::string& filename, AudioGroupID groupID) = 0;
	virtual bool Unload(const std::string& filename) noexcept = 0;
	virtual void SetVolume(AudioGroupID groupID, float volume) noexcept = 0;
	virtual bool Play(const std::string& filename) = 0;
	virtual PlayState GetPlayState(const std::string& filename) = 0;
